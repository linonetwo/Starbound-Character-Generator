...
Painter.prototype.repaint = function( config, currentSettings, imageInfo )
{
	...
	if ( pantsArmorName == "extra" )
		pantsArmorSheet	= imageInfo["extra-" + currentSettings.gender + "Pants"];
	else
		pantsArmorSheet	= imageInfo[currentSettings.species + "-armor-" + pantsArmorName + "-" + currentSettings.gender + "Pants"];
	...
	if ( speciesVars.headOptionAsHairColor )
		...
		hairPalette		= hairPalette.concat( bodyPalettes );
	...
	// Paint erryting!
	// Back Arm
	this.paintToCanvas( backArmSheet, poseConfig.sleeveIndex[0], poseConfig.sleeveIndex[1], poseConfig.sleeveOffset[0], poseConfig.sleeveOffset[1], bodyPalettes, this.backArmCanvas );
	
	// Back Sleeve Armor
	if ( backSleeveSheet != null )
		this.paintToCanvas( backSleeveSheet, poseConfig.sleeveIndex[0], poseConfig.sleeveIndex[1], poseConfig.sleeveOffset[0], poseConfig.sleeveOffset[1], shirtPalette, this.backSleeveCanvas );
	else
		this.backSleeveCanvas.getContext( "2d" ).clearRect( 0, 0, this.SPRITE_SIZE, this.SPRITE_SIZE );
	
	// Body
	this.paintToCanvas( bodySheet, poseConfig.bodyIndex[0], poseConfig.bodyIndex[1], 0, 0, bodyPalettes, this.bodyCanvas );
	
	// Pants Armor
	if ( pantsArmorSheet != null )
		this.paintToCanvas( pantsArmorSheet, poseConfig.bodyIndex[0], poseConfig.bodyIndex[1], 0, 0, pantsPalette, this.pantsCanvas );
	else
		this.pantsCanvas.getContext( "2d" ).clearRect( 0, 0, this.SPRITE_SIZE, this.SPRITE_SIZE );
	
	// Shirt Armor
	if ( shirtArmorSheet != null )
		this.paintToCanvas( shirtArmorSheet, poseConfig.chestIndex[0], poseConfig.chestIndex[1], poseConfig.chestOffset[0], poseConfig.chestOffset[1], shirtPalette, this.shirtCanvas );
	else
		this.shirtCanvas.getContext( "2d" ).clearRect( 0, 0, this.SPRITE_SIZE, this.SPRITE_SIZE );
	
	// Hair
	if ( helmetArmorSheet == null || currentSettings.helmetOption == currentSettings.helmetOptionMax - 1 )	
		this.paintToCanvas( accessorySheet, currentSettings.hairOption, hairOffset, poseConfig.hairOffset[0], poseConfig.hairOffset[1], hairPalette, this.hairCanvas );
	else
		//this.hairCanvas.getContext( "2d" ).clearRect( 0, 0, this.SPRITE_SIZE, this.SPRITE_SIZE );
		this.paintWithMask( accessorySheet, currentSettings.hairOption, hairOffset, poseConfig.hairOffset[0], poseConfig.hairOffset[1], hairPalette, this.hairCanvas, helmetMaskSheet );
		
	// Head
	if ( speciesVars.headOptionAsFacialHair )
		this.paintToCanvas( accessorySheet, currentSettings.headOption, headOffset, poseConfig.hairOffset[0], poseConfig.hairOffset[1], bodyPalettes, this.headCanvas );
	else
		this.headCanvas.getContext( "2d" ).clearRect( 0, 0, this.SPRITE_SIZE, this.SPRITE_SIZE );
	
	// Alt
	if ( speciesVars.altOptionAsFacialMask )
		this.paintToCanvas( accessorySheet, currentSettings.altOption, altOffset, poseConfig.hairOffset[0], poseConfig.hairOffset[1], bodyPalettes, this.altCanvas );
	else
		this.altCanvas.getContext( "2d" ).clearRect( 0, 0, this.SPRITE_SIZE, this.SPRITE_SIZE );
	
	// Helmet
	if ( helmetArmorSheet != null && currentSettings.helmetOption < currentSettings.helmetOptionMax - 1 )
		this.paintToCanvas( helmetArmorSheet, 1, 0, poseConfig.hairOffset[0], poseConfig.hairOffset[1], helmetPalette, this.helmetCanvas );
	
	
	// Front Arm
	this.paintToCanvas( frontArmSheet, poseConfig.sleeveIndex[0], poseConfig.sleeveIndex[1], poseConfig.sleeveOffset[0], poseConfig.sleeveOffset[1], bodyPalettes, this.frontArmCanvas );
	
	// Front Sleeve Armor
	if ( frontSleeveSheet != null )
		this.paintToCanvas( frontSleeveSheet, poseConfig.sleeveIndex[0], poseConfig.sleeveIndex[1], poseConfig.sleeveOffset[0], poseConfig.sleeveOffset[1], shirtPalette, this.frontSleeveCanvas );
	else
		this.frontSleeveCanvas.getContext( "2d" ).clearRect( 0, 0, this.SPRITE_SIZE, this.SPRITE_SIZE );
};


Painter.prototype.paintToCanvas = function( sheet, offsetX, offsetY, nudgeX, nudgeY, paletteSwaps, tempCanvas )
{
	...
	var posX = this.SPRITE_SIZE -1, posY = this.SPRITE_SIZE - 1;
	...
	while ( px-- )
	{
		...
		if ( imageData[4*px+3] != 0 )
		{
			...
			for ( swap in paletteSwaps )
			{
				...
				for ( color in paletteSwaps[swap] )
				{
					if ( color == colorHex )
					{
			...
			this.CONTEXT.fillRect( ( posX + nudgeX ) * this.PREVIEW_MULTI, ( posY + nudgeY ) * this.PREVIEW_MULTI, this.PREVIEW_MULTI, this.PREVIEW_MULTI );
		}
...
Painter.prototype.paintWithMask = function( sheet, offsetX, offsetY, nudgeX, nudgeY, paletteSwaps, tempCanvas, mask )
{
	console.log( "Painting sprite with mask at " + offsetX * this.SPRITE_SIZE + "," + offsetY * this.SPRITE_SIZE + " with " + paletteSwaps.length + " swaps, of sheet " + sheet.src );
	var tempCtx			= tempCanvas.getContext( "2d" );
...
Painter.prototype.rgbToHex = function( r, g, b  )
{
